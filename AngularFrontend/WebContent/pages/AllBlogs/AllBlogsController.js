app.controller("AllBlogsController",function($scope,$http,$rootScope,$route)	
					...
					console.log("username in allblog controller:"+$rootScope.uname);
							 $http.get("http://localhost:8087/Backend/viewAllBlogs")
							 ...
							 { 
						       ...
						       var like=
								...
								console.log("postedby:"+$rootScope.uname);
								 $http.put('http://localhost:8087/Backend/updateBlog',like);
								 $http.get("http://localhost:8087/Backend/viewBlogs")
								    .then(function (response) {
					 ...
					 $http.get("http://localhost:8087/Backend/viewComments/"+$scope.viewcomments.blog_id)
								    .then(function (response) {
						...
						$scope.comment=function(allblogs)
						{
							...
							$http.post('http://localhost:8087/Backend/addComment',comment);
							 $http.get("http://localhost:8087/Backend/viewAllBlogs")
							    .then(function (response) {
							    	
							    	$scope.blogs = response.data;
							    	$scope.message = "comments added successfully";
							    	console.logs(blogs);
							    	console.log("data:"+response.data);
							    });
