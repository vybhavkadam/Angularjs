...
app.controller("AdminForumController", function($scope, $http, $rootScope) {
	...
	$http.get("http://localhost:8087/Backend/viewForums").then(
			function(response) {
	...
	$scope.addForum = function(newForum) {
		...
		var res = $http.post('http://localhost:8087/Backend/addForum',
				dataObj);
		$http.get("http://localhost:8087/Backend/viewForums").then(
				function(response) {
					$scope.forums = response.data;
	...
	$scope.approveForum = function() {
		...
		$http.put("http://localhost:8087/Backend/updateForum", edit);
		$http.get("http://localhost:8087/Backend/viewForums").then(
				function(response) {
					$scope.forums = response.data;
					console.log("data:" + response.data);
